...
const ERC1155ReceiverDelegate = artifacts.require(
  'ERC1155ReceiverDelegate.sol'
);
const ERCXXXXReceiverDelegate = artifacts.require(
  'ERCXXXXReceiverDelegate.sol'
);

const SignaturesERC1155ReceiverDelegate =
  'onERC1155Received(address,address,uint256,uint256,bytes)onERC1155BatchReceived(address,address,uint256[],uint256[],bytes)setShouldReject(bool)';
const SignaturesERCXXXXReceiverDelegate =
  'onERCXXXXReceived(address,address,uint256,uint256,bytes)onERCXXXXBatchReceived(address,address,uint256[],uint256[],bytes)setShouldRejectXXXX(bool)setShouldRejectClash(bool)';
...
function verifyTransferEvent(tx, id, from, to, quantity, operator) {
  ...
  if (eventCount === 0) assert(false, 'Missing Transfer Event');
  else assert(eventCount === 1, 'Unexpected number of Transfer events');
}
...
async function testSafeTransferFrom(
  ...
  quantity,
  data,
  gasMessage = 'testSafeTransferFrom'
) {
  ...
  tx = await mainContract.safeTransferFrom(from, to, id, quantity, data, {
    from: operator
  });
  ...
  if (from !== to) {
    assert.strictEqual(
      preBalanceFrom.sub(quantity).toNumber(),
      postBalanceFrom.toNumber()
    );
    assert.strictEqual(
      preBalanceTo.add(quantity).toNumber(),
      postBalanceTo.toNumber()
    );
  } else {
...
function verifyTransferEvents(tx, ids, from, to, quantities, operator) {
  ...
  assert(
    totalIdCount === ids.length,
    'Unexpected number of Transfer events found ' +
      totalIdCount +
      ' expected ' +
      ids.length
  );
...
async function testSafeBatchTransferFrom(
  operator,
  from,
  to,
  ids,
  quantities,
  data,
  gasMessage = 'testSafeBatchTransferFrom'
) {
  let preBalanceFrom = [];
  let preBalanceTo = [];
  ...
  tx = await mainContract.safeBatchTransferFrom(
    from,
    to,
    ids,
    quantities,
  ...
  for (let i = 0; i < ids.length; ++i) {
    if (from !== to) {
      assert.strictEqual(
        preBalanceFrom[i].sub(quantities[i]).toNumber(),
        postBalanceFrom[i].toNumber()
      );
      assert.strictEqual(
        preBalanceTo[i].add(quantities[i]).toNumber(),
        postBalanceTo[i].toNumber()
      );
    } else {
      assert.strictEqual(
        preBalanceFrom[i].toNumber(),
        postBalanceFrom[i].toNumber()
      );
    }
  }
...
contract(
  'ERC1155ProxyTest - tests sending 1155 items to an ERC1538 supported proxy contract.',
  (accounts) => {
    before(async () => {
      user1 = accounts[1];
      user2 = accounts[2];
    ...
    it('Create initial items', async () => {
      ...
      tx = await mainContract.create(hammerQuantity, hammerUri, {
        from: user1
      });
    ...
    it('setup proxy with delegate and try same send', async () => {
      await receiverContract.updateContract(
        receiverDelegateERC1155.address,
        SignaturesERC1155ReceiverDelegate,
        'Adding in ERC1155ReceiverDelegate'
      );
      await testSafeTransferFrom(
        ...
        1,
        web3.utils.fromAscii('SomethingMeaningfull'),
        'testSafeTransferFrom receiver 1155'
      );
    });
    ...
    it('remove delegate link and try batchTransfer', async () => {
      await receiverContract.updateContract(
        zeroAddress,
        SignaturesERC1155ReceiverDelegate,
        'Invalidating ERC1155ReceiverDelegate delegate'
      );
      await expectThrow(
        mainContract.safeBatchTransferFrom(
          user1,
          receiverContract.address,
          idSet,
          quantities,
    ...
    it('restore delegate link and try batchTransfer again also testing delegate reject', async () => {
      await receiverContract.updateContract(
        receiverDelegateERC1155.address,
        SignaturesERC1155ReceiverDelegate,
        'Adding in ERC1155ReceiverDelegate again'
      );

      let proxy1155Delegate = await ERC1155ReceiverDelegate.at(
        receiverContract.address
      );
      ...
      await expectThrow(
        ...
        'onERC1155BatchReceived: transfer not accepted'
      );
      await proxy1155Delegate.setShouldReject(false);
      await testSafeBatchTransferFrom(
        user1,
        user1,
        receiverContract.address,
        idSet,
        quantities,
        web3.utils.fromAscii(''),
        'testSafeBatchTransferFrom receiver 1155'
      );
    });

    it('show how a future delegate can clash with previous delegates data', async () => {
      await receiverContract.updateContract(
        receiverDelegateERCXXXX.address,
        SignaturesERCXXXXReceiverDelegate,
        'Adding in ERCXXXXReceiverDelegate'
      );

      let proxyXXXXDelegate = await ERCXXXXReceiverDelegate.at(
        receiverContract.address
      );
      ...
      await expectThrow(
        ...
        'onERC1155Received: transfer not accepted'
      );
    ...
    it('attempt direct call to a delegate', async () => {
      await expectThrow(
        receiverDelegateERC1155.onERC1155Received(
          zeroAddress,
          zeroAddress,
          0,
          0,
